using UnityEngine;
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        private void Move()
        {
            ...
            if(Focusing)
            {
                _baseParameter = Mathf.Clamp01(_baseParameter + Time.deltaTime * 4f);
                _focusingParameterXZ = Vector3.Lerp(_focusingParameterXZ, new Vector3(_rawMoveInputBlend.x, 0, _rawMoveInputBlend.y) * _speed, Time.deltaTime * 4f);
            }else
            {
                _baseParameter = Mathf.Clamp01(_baseParameter - Time.deltaTime * 4f);
                _focusingParameterXZ = Vector3.Lerp(_focusingParameterXZ, Vector3.forward * _speed, Time.deltaTime * 4f);
            }
            _animator.SetFloat(_animIDSpeed, _baseParameter);
            _animator.SetFloat(_animIDXAxis, _focusingParameterXZ.x);
            _animator.SetFloat(_animIDZAxis, _focusingParameterXZ.z);
        }
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                // Jump
                if (_input._jump.IsPressed() && _jumpTimeoutDelta <= 0.0f)
                {
